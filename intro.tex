...
sync. vs. async. sync: high scheduling cost(sync. barrier) and low communication cost; while async: low scheduling cost (coordination free) and high communication cost (no wait, no idle, every one compute and communicate) and sometimes low computation efficiency (stale comptuation). actually is to find a balance point between scheduling cost, communication cost, computation efficiency.
...
make more general aggregation, synchronous is required for correct aggregation (from algorithm aspect), async is general for parallel execution (from parallel computing aspect). use sync in parallel to guarantee correctness. monotonic async can guarantee correctness, so sync is not required and computation effiency is high
